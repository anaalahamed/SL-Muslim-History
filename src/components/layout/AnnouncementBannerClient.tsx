...
export default function AnnouncementBannerClient({ announcement }: Props) {
  ...
  useEffect(() => {
    if (sessionStorage.getItem(`slmh_announce_dismissed_${announcement.text}`)) setDismissed(true)
  }, [announcement.text])

  function dismiss() {
    sessionStorage.setItem(`slmh_announce_dismissed_${announcement.text}`, '1')
    setDismissed(true)
  }
