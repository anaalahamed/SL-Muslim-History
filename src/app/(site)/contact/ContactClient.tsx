...
const faqs = [
  { q: 'How can I submit an article?', a: 'Use the contact form above, select "Submit an Article" as the reason, and briefly describe your topic. Our editorial team will review and get back to you with submission guidelines.' },
  { q: 'Can I share old family photographs?', a: 'Yes! Historical photographs are invaluable to us. Select "Share Historical Photos" and we will guide you through the donation or archiving process.' },
  { q: 'Is the content free to read?', a: 'All content on SL Muslim History is completely free. We believe history should be accessible to everyone in the community.' },
  { q: 'How do I report an error in an article?', a: 'Select "Report an Error" in the contact form and include the article title and what you believe is incorrect. We take accuracy seriously and will review promptly.' },
  { q: 'Can organisations partner with you?', a: 'We welcome partnerships with universities, cultural organisations, and community groups. Select "Partnership / Collaboration" and tell us about your organisation.' },
]
...
function FAQ() {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)', background: 'white' }}>
      <div className="px-6 py-4" style={{ background: 'var(--green-light)', borderBottom: '1px solid var(--border)' }}>
...
export default function ContactClient() {
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      <PageHero badge="Contact Us" title="Get in Touch" subtitle="Have a question, want to contribute, or share a piece of history? We'd love to hear from you." />
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="flex-1 min-w-0">
            <AnimateIn direction="up">
              <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)', background: 'white' }}>
                <div className="px-8 py-6" style={{ background: 'var(--green-light)', borderBottom: '1px solid var(--border)' }}>
                ...
                {status === 'sent' ? (
                  <div className="p-12 text-center">
                    ...
                    <button onClick={() => { setForm({ name: '', email: '', reason: '', message: '' }); setStatus('idle') }} className="px-6 py-2.5 rounded-xl font-bold text-sm" style={{ background: 'var(--green)', color: 'white' }}>
                  ...
                  <form onSubmit={handleSubmit} className="p-8 space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div>
                        ...
                        <input type="text" required placeholder="Mohamed Ali" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-4 py-3 rounded-xl text-sm outline-none" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)', transition: 'border-color 0.2s, box-shadow 0.2s' }} onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--green)'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(74,158,31,0.12)' }} onBlur={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.boxShadow = 'none' }} />
                      </div>
                      <div>
                        <label className="block text-xs font-bold mb-1.5" style={{ color: 'var(--dark)' }}>Email Address <span style={{ color: '#dc2626' }}>*</span></label>
                        <input type="email" required placeholder="you@example.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full px-4 py-3 rounded-xl text-sm outline-none" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)', transition: 'border-color 0.2s, box-shadow 0.2s' }} onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--green)'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(74,158,31,0.12)' }} onBlur={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.boxShadow = 'none' }} />
                      </div>
                    </div>

                    <div>
                      <label className="block text-xs font-bold mb-2" style={{ color: 'var(--dark)' }}>Reason for Contact</label>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {reasons.map((r) => (
                          <button key={r.label} type="button" onClick={() => setForm({ ...form, reason: r.label })} className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold text-left transition-all" style={{ border: `1px solid ${form.reason === r.label ? 'var(--green)' : 'var(--border)'}`, background: form.reason === r.label ? 'var(--green-light)' : 'var(--bg)', color: form.reason === r.label ? 'var(--green)' : 'var(--muted)' }}>
                            <span>{r.icon}</span>
            ...
            <AnimateIn direction="up" delay={100}>
              <div className="rounded-2xl p-6 mt-6" style={{ border: '1px solid var(--border)', background: 'white' }}>
                ...
                <div className="flex flex-col sm:flex-row gap-0">
                  {[
                    ...
                    <div key={item.step} className="flex sm:flex-col flex-1 items-start sm:items-center sm:text-center gap-4 sm:gap-2 relative">
                      {i < 2 && <div className="hidden sm:block absolute top-5 left-1/2 w-full h-0.5" style={{ background: 'var(--border)', zIndex: 0 }} />}
                      <div className="relative z-10 w-10 h-10 rounded-full flex items-center justify-center text-xl flex-shrink-0" style={{ background: 'var(--green-light)', border: '2px solid var(--green)' }}>{item.icon}</div>
          ...
          <div className="lg:w-80 flex-shrink-0">
            <div className="space-y-6 lg:sticky lg:top-6">
              <AnimateIn direction="right" delay={100}>
                <div className="rounded-2xl p-5 space-y-4" style={{ border: '1px solid var(--border)', background: 'white' }}>
                  <h3 className="font-extrabold text-sm flex items-center gap-2" style={{ color: 'var(--dark)' }}>
                    <span className="w-1 h-5 rounded-full inline-block" style={{ background: 'var(--green)' }} />
                    Contact Information
                  </h3>
                  {contactInfo.map((info) => (
                    <div key={info.label} className="flex items-start gap-3 p-3 rounded-xl" style={{ background: info.bg }}>
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center text-lg flex-shrink-0" style={{ background: 'white' }}>{info.icon}</div>
              ...
              <AnimateIn direction="right" delay={200}>
                <div className="rounded-2xl p-5" style={{ border: '1px solid var(--border)', background: 'white' }}>
                  ...
                  <div className="space-y-2">
                    {activeSocials.map((s) => (
                      <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-xl transition-all" style={{ border: '1px solid var(--border)', minWidth: 0, overflow: 'hidden' }} onMouseEnter={(e) => { e.currentTarget.style.background = s.color + '12'; e.currentTarget.style.borderColor = s.color }} onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'var(--border)' }}>
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-white" style={{ background: s.color }}>{s.icon}</div>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div className="text-xs font-bold" style={{ color: 'var(--dark)' }}>{s.label}</div>
                          <div className="text-xs" style={{ color: 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>{s.href}</div>
                        </div>
                        <span style={{ fontSize: '12px', color: 'var(--muted)', flexShrink: 0 }}>→</span>
                      </a>
                    ))}
                    {activeSocials.length === 0 && <p className="text-xs" style={{ color: 'var(--muted)' }}>No social links configured yet.</p>}
                  </div>
                </div>
              </AnimateIn>
              ...
              <AnimateIn direction="right" delay={300}>
                <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
                  <div className="h-44 flex flex-col items-center justify-center gap-2" style={{ background: 'var(--green-light)' }}>
                    <span className="text-4xl">🗺️</span>
                    {config.location && <span className="text-sm font-semibold" style={{ color: 'var(--green-dark)' }}>{config.location}</span>}
