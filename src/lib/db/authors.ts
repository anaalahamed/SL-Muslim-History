...
export async function saveAuthor(author: Partial<Author>, client?: SupabaseClient): Promise<{ data: Author | null; error: string | null }> {
  const db = client ?? supabase
  if (!db) return { data: null, error: 'Supabase not configured' }
  if (author.id) {
    const { id, ...rest } = author
    const { data, error } = await db.from('authors').update(rest).eq('id', id).select().single()
    return { data: data as Author | null, error: error?.message ?? null }
  }
  const payload = { ...author, created_at: new Date().toISOString() }
  const { data, error } = await db.from('authors').insert(payload).select().single()
  return { data: data as Author | null, error: error?.message ?? null }
}

export async function deleteAuthor(id: string, client?: SupabaseClient): Promise<string | null> {
  const db = client ?? supabase
